        ...
        if form.is_valid():
            ...
            class ReturnActions(object):
                ...
                def _continue(self):
                    if self.plan:
                        return HttpResponseRedirect('%s?from_plan=%s' % (
                                reverse('tcms.apps.testcases.views.edit', args=[self.case.case_id, ]),
                                self.plan.plan_id
                            )
                        )

                    return HttpResponseRedirect(
                        reverse('tcms.apps.testcases.views.edit', args=[tc.case_id, ]),
                    )
                ...
                def _returntocase(self):
                    if self.plan:
                        return HttpResponseRedirect('%s?from_plan=%s' % (
                                reverse('tcms.apps.testcases.views.get', args=[self.case.pk, ]),
                                self.plan.plan_id
                            )
                        )
                ...
                def _returntoplan(self):
                    ...
                    return HttpResponseRedirect('%s#reviewcases' %
                        reverse('tcms.apps.testplans.views.get', args=[self.plan.pk, ]),
                    )
    ...
    if request.GET.get('from_plan'):
        try:
            ...
            return HttpResponse(Prompt.render(
                ...
                next = reverse('tcms.apps.testcases.views.get', args = [case_id,]),
            ))
    else:
        tp = None
