...
class BaseRunForm(forms.Form):
    ...
    estimated_time = TimedeltaFormField(required = False)
...
def NewCaseRunForm(BaseCaseRunForm):
    pass

class EditCaseRunForm(BaseCaseRunForm):
    pass
...
class XMLRPCNewCaseRunForm(BaseCaseRunForm):
    ...
    def clean_assignee(self):
        ...
        if not data:
            if self.cleaned_data.get('case') and self.cleaned_data['case'].default_tester_id:
